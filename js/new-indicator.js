...
require([
    ...
    const yearDefinition2 = new DimensionalDefinition({
        variableName: 'plantheatstress_count',
        dimensionName: 'Year',
        values: [2024],
        isSlice: true
    ...
    let mosaicRule = new MosaicRule({
        multidimensionalDefinition: [yearDefinition, yearDefinition2]
    });
    ...
    let indicatorLayerPopupTemplate2 = {
        title: '',
        content: '<b>{Raster.ItemPixelValue}</b> total days in <b>{Year}</b> when the maximum temperature is greater than 25\u00B0C',
        fieldInfos: [{
            fieldName: 'Raster.ItemPixelValue',
    ...
    const layerOptions = [
      { value: 'layer1', text: 'Ensemble5' },
      { value: 'layer2', text: 'Ensemble4' },
    ];
    layerOptions.forEach(option => {
      const optionElement = document.createElement('option');
      optionElement.value = option.value;
      optionElement.text = option.text;
      layerSelect.add(optionElement);
    });
    view.ui.add(layerSelect, 'top-right');


// create popup templates for each layer
const popupTemplates = {
layer1: indicatorLayerPopupTemplate,
layer2: indicatorLayerPopupTemplate,
};

layerSelect.addEventListener('change', event => {
const selectedLayer = event.target.value;
let mosaicRuleCopy = new MosaicRule({
    multidimensionalDefinition: []
});
switch (selectedLayer) {
    case 'layer1':
        // Set mosaic rule for selected layer
        mosaicRuleCopy.multidimensionalDefinition.push(yearDefinition);
        // Set popup template for selected layer
        indicatorLayer.popupTemplate = popupTemplates.indicatorLayerPopupTemplate;
        // Set layer visibility
        indicatorLayer.visible = true;
        // Hide other layers
        indicatorLayer2.visible = false;
        break;
    case 'layer2':
        // Set mosaic rule for selected layer
        mosaicRuleCopy.multidimensionalDefinition.push(yearDefinition2);
        // Set popup template for selected layer
        indicatorLayer2.popupTemplate = popupTemplates.indicatorLayerPopupTemplate;
        // Set layer visibility
        indicatorLayer2.visible = true;
        // Hide other layers
        indicatorLayer.visible = false;
        break;

}
// Update mosaic rule for selected layer
if (selectedLayer !== '') {
    switch (selectedLayer) {
        case 'layer1':
            indicatorLayer.mosaicRule = mosaicRuleCopy;
            break;
        case 'layer2':
            indicatorLayer2.mosaicRule = mosaicRuleCopy;
            break;
    }
}
});
